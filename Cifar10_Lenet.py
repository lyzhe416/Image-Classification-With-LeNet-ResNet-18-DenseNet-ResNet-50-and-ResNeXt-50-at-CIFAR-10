# -*- coding: utf-8 -*-
###############################################################################
# Mission Description : Lenet for CIFAR10 Classification
...
from tensorflow.keras.layers    import Conv2D, MaxPool2D, AveragePooling2D, GlobalAveragePooling2D, Flatten
...
import shutil
###############################################################################
real_time_epochs   = 250

img_rows, img_cols = 32, 32
img_channels       = 3
weight_decay       = 1e-4

mother_path = './Storage_Lenet/'
Order       = True
###############################################################################
...
def Resnet(img_input, cat_num, block_list):
    # img_nchannels = 3
    x = conv(img_input, output_filters=6 , kernel_size=5, strides=1)
    x = Activation('relu')(x)
    x = MaxPool2D(pool_size=2, strides=2 , padding='valid')(x)
    
    x = conv(x        , output_filters=16, kernel_size=5, strides=1)
    x = Activation('relu')(x)
    x = MaxPool2D(pool_size=2, strides=2 , padding='valid')(x)
    
    x = conv(x        , output_filters=64, kernel_size=3, strides=1)
    
    x = Flatten()(x)
    
    x = DenseLayer(x, 128, 'relu')
    x = DenseLayer(x, 84 , 'relu')
