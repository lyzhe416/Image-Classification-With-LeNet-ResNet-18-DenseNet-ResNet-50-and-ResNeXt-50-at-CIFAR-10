# -*- coding: utf-8 -*-
###############################################################################
# Mission Description : Resnet18 for CIFAR10 Classification
...
# Functions to Define the Topology Structure of Convolution Layers in Resnet Blocks
def ResnetBlock(x, inchannels, strides, residual_path):
    residual  = x
    nchannels = strides*inchannels
    x = conv(x, output_filters=nchannels, kernel_size=3, strides=strides)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = conv(x, output_filters=nchannels, kernel_size=3, strides=1)
    x = BatchNormalization()(x)
    if residual_path == True:
        residual = conv(residual, output_filters=nchannels, kernel_size=1, strides=strides)
...
def Resnet(img_input, cat_num, block_list):
    # img_nchannels = 3
    nchannels    = 64
    numBigBlock  = len(block_list)
    x = conv(img_input, output_filters=nchannels, kernel_size=3, strides=1)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    
    for indBigBlock in range(0, 1):
        for numSmallBlock in range(0, block_list[indBigBlock]):
            x, nchannels = ResnetBlock(x, inchannels=nchannels, strides=1, residual_path=True)
    
    for indBigBlock in range(1, numBigBlock):
        x, nchannels = ResnetBlock(x, inchannels=nchannels, strides=2, residual_path=True)
        for numSmallBlock in range(1, block_list[indBigBlock]):
            x, nchannels = ResnetBlock(x, inchannels=nchannels, strides=1, residual_path=True)
    
    hidden_neuron_number = 320
...
output    = Resnet(img_input, cat_number, [2,2,2,2])
