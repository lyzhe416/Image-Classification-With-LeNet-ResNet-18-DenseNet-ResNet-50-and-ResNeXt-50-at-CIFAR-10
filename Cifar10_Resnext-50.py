# -*- coding: utf-8 -*-
###############################################################################
# Mission Description : ResNeXt for CIFAR10 Classification
...
# Function of Grouped Convolution
def group_conv(x, grouped_channels, kernel_size, cardinality, strides):
    group_list = []
    for i in range(cardinality):
        x_grouped = Lambda(lambda t: t[:, :, :, i*grouped_channels : (i+1)*grouped_channels]) (x)
        x_grouped = conv(x_grouped, output_filters=grouped_channels, kernel_size=kernel_size, strides=1)
        group_list.append(x_grouped)
    x_concat = concatenate(group_list, axis=-1)
    return x_concat
        

# Functions to Define the Topology Structure of Bottleneck Blocks Layers in ResNeXt
def BottleNeckBlock(x, nchannels, cardinality, strides, residual_path):
    residual  = x
    x = conv(x, output_filters=1*nchannels, kernel_size=1, strides=strides)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x = group_conv(x, grouped_channels=int(nchannels/cardinality), kernel_size=3,
...
# Functions to Define the Topology Structure of Bottleneck Blocks Layers in ResNeXt
def ResNeXtBlock(x, nchannels, cardinality, strides, residual_path, block_dense):
    x, outchannels = BottleNeckBlock(x, nchannels, cardinality, strides=strides, residual_path=True)
    for i in range(1, block_dense):
        x, outchannels = BottleNeckBlock(x, nchannels, cardinality, strides=1, residual_path=True)
    return x, outchannels

# Define the Topology Structure of a Complete Resnet (Resnet18)
def ResNeXt(img_input, cat_num, block_list):
    # img_nchannels = 3
    nchannels   = 64
    ...
    for indBlock in range(0, 1):
        x, outchannels = ResNeXtBlock(x, outchannels, cardinality=cardinality, strides=1,
                                      residual_path=True, block_dense=block_list[indBlock]) 
    for indBlock in range(1, numBlock):
        x, outchannels = ResNeXtBlock(x, outchannels, cardinality=cardinality, strides=2,
                                      residual_path=True, block_dense=block_list[indBlock])
...
output    = ResNeXt(img_input, cat_number, [3, 4, 6, 3])
